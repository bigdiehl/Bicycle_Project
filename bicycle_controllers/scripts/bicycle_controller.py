...
# Controllers
from lqr import LQR_Controller
...
class BicycleController():
    ...
    def __init__(self):
        ...
        self.js_sub = rospy.Subcriber("joint_states", JointState, self.ReadJointStates)
        self.tf_listener = tf.TransformListener()

        # Subscribe to bicycle commands generated by seperate entity
        self.command_sub = rospy.Subcriber("bicycle/command", BicycleCmd, self.ReceiveCommand)
    ...
    def Run(self):
        ...
        # TODO - condition this to only run if Gazebo is not paused?

        while not rospy.is_shutdown():
            try:
                if self.first_states_received:
                    [torque_cmd, velocity_cmd, position_cmd] = self.controller(self.inputs, self.commands)
                    
                    if torque_cmd is not None:
                        self.tau_pub.publish(torque_cmd)

                    if velocity_cmd is not None:
                        self.vel_pub.publish(torque_cmd)
